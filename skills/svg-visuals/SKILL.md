...
- **흐름 애니메이션**(`d-flow`)은 데이터가 실제로 흐르는 경로에만, 한 그림에
  최대 3개. 정적 선 위에 겹쳐 깐다. 노드는 절대 애니메이션하지 않는다 —
  박스가 움직이면 글을 읽을 수 없다. 자세한 건 `design-tokens.md`의
  "흐름 애니메이션" 절.
...
- 박스 높이 기본 **48** (한 줄), **68** (두 줄), 모서리 반경 **10**
...
- [ ] `d-flow` 아래에 정적 선이 함께 있는가 (애니메이션이 꺼져도 보이는가)
- [ ] `d-flow`에 `marker-end`를 붙이지 않았는가 (화살표가 깜빡인다)
- [ ] 움직이는 것이 연결선뿐인가 (노드·텍스트는 정지)
