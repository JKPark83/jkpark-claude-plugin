...
스킬 14개 + 에이전트 2개 + 훅 1개 + 상태줄(옵션)로 구성된다.
...
| 스킬 | 하는 일 | 트리거 예시 |
|------|---------|-------------|
| `us-monthly-dividend` | 미국 월배당 포트폴리오 설계·분석 — 1~12월 배당 달력, 시장 상황(VIX·미 10년물 + 병렬 애널리스트 서브에이전트) 기반 커버드콜 한도·종목 수 제안, 추가납입 리밸런싱(buy-only 배분·5/25 밴드, 매도는 명시 동의 시만, 기존 계좌는 이전에 내보낸 구글시트에서 재로딩 가능), 세후(15% 원천징수) 현금흐름, 배당컷 OK/WARN/REVIEW 판정 (yfinance 스크립트 기반), 17종 후보군(REIT·BDC·커버드콜·우선주·배당성장 ETF)에서 유형 다양성 우선 선정 + backtest-analyst 에이전트로 제안 포트폴리오 백테스트 필수 수행(SPY·SCHD 벤치마크, 구성 근거 섹션에 결과 포함), obs 저장 + 스타일드 구글시트(색상 헤더·줄무늬·통화/퍼센트 서식, openpyxl xlsx 변환) 내보내기 선택 | "월배당 포트폴리오 짜줘", "1000만원 추납 재조정" |
...
| `backtest-analyst` (agent) | us-monthly-dividend의 결정적 백테스트 스크립트를 실행해 SPY·SCHD 대비 성과 표와 한국어 해석을 돌려주는 Sonnet 서브에이전트 |
